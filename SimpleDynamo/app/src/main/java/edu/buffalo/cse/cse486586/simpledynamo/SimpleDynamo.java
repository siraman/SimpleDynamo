...
public class SimpleDynamo extends ContentProvider {
...
//            replicateData(key, value, coordinator);
//            replicateToSuccessor(key,value,coordinator,replicationList[0]);
...
//                replicateToSuccessor(key,value,coordinator,replicationList[0]);
    ...
    private void replicateToSuccessor(String key, String value, String coordinator, String successor) {
        try {
            Socket socket = new Socket(InetAddress.getByAddress(new byte[]{10, 0, 2, 2}),
                    Integer.parseInt(successor));
//            socket.setSoTimeout(3000);
            PrintStream printStream = new PrintStream(socket.getOutputStream());
            String messageToSend = INSERT_SUCCESSOR + ":" + coordinator +":" + key + ":" + value;
            Log.e("conCoordForInsert", "message to send " + messageToSend);
            printStream.println(messageToSend);
...
//            String[] msg = messageReceived.split(":");
    ...
    private void replicateData(String key, String value, String coordinator) {
        String message = key + ":" + value;
        new ClientTask().executeOnExecutor(AsyncTask.SERIAL_EXECUTOR, message, REPLICATION, coordinator);
    }
...
//            Log.e("else query", "contactCoordinatorForQuery");
//            return contactCoordinatorForQuery(selectionItems);
            String coordinator = dynamoRing.getCoordinator(genHash(selectionItems[0]));
            String selectionItem = SimpleDynamoContract.MessageEntry.COLUMN_KEY + " = ?";
            String[] selectionArg = new String[]{selectionItems[0]};
            if (MY_PORT.equals(coordinator)) {
                cursor = dbReader.query(SimpleDynamoContract.MessageEntry.TABLE_NAME, null, selectionItem, selectionArg,
                        null, null, null);
            }
            else {
    ...
    private Cursor contactCoordinatorForQuery(String[] selection) {
        String selectionItems = selection[0];
        MatrixCursor cursor = null;
        Cursor myCursor = null;
        String selectionItem = SimpleDynamoContract.MessageEntry.COLUMN_KEY + " = ?";
        String[] selectionArgs = new String[]{selectionItems};
        String coordinator = dynamoRing.getCoordinator(genHash(selectionItems));
        Log.e("contactCoorForQuery", "coordinator " + coordinator);
        if (MY_PORT.equals(coordinator)) {
            myCursor = dbReader.query(SimpleDynamoContract.MessageEntry.TABLE_NAME, null, selectionItem, selectionArgs,
                    null, null, null);
            return myCursor;
//            return contactReplicasForQuery(selectionItems,coordinator);
        } else {
//            try {
//                Socket socket = new Socket(InetAddress.getByAddress(new byte[]{10, 0, 2, 2}),
//                        Integer.parseInt(coordinator));
...
//                PrintStream printStream = new PrintStream(socket.getOutputStream());
//                String messageToSend = COORDINATOR_QUERY + ":" + selection[0];
...
//                io.printStackTrace();
//            }
            return contactCoordinator(selectionItems,coordinator);
        }
//        return cursor;
    }
...
//                        Log.e("ServerTask", "COORDINATOR_REPLICATION");
//                        ContentValues values = new ContentValues();
//                        values.put(KEY, messageParts[1]);
//                        values.put(VALUE, messageParts[2]);
//                        Log.e("ServerTask", "COORDINATOR_REPLICATION data " + messageParts[1] + " " + messageParts[2]);
//                        insert(null, values);
                        ContentValues values = new ContentValues();
                        String coordinator = messageParts[1];
                        values.put(KEY, messageParts[2]);
...
//                            break;
//                        else
//                            replicateToSuccessor(messageParts[2],messageParts[3],
//                                    coordinator,dynamoRing.getSuccessor(MY_PORT));
//                        break;
                    } else if (messageParts[0].equals(COORDINATOR_QUERY)) {
                        Log.e("ServerTask", "COORDINATOR_QUERY");
//                        String selection = messageParts[1];
//                        Cursor cursor = dbReader.query(SimpleDynamoContract.MessageEntry.TABLE_NAME, null, selection, selectionArgs,
//                                null, null, null);
                        String selectionItem = SimpleDynamoContract.MessageEntry.COLUMN_KEY + " = ?";
                        ...
                        if (fromPort.equals(dynamoRing.getPredecessor(MY_PORT))) {
                            Log.e("server rec","sending pred12 data");
                            if (!predecessor1Data.isEmpty() && !predecessor2Data.isEmpty()) {
                                Log.e("RECOVERY","sending predecessor1 and predecessor2 data");
                                for (Map.Entry<String, String> entry : predecessor1Data.entrySet()) {
                                    if (messagePart1.isEmpty()) {
                                        messagePart1 += RECOVERY_ACK + ":" + MY_PORT + ":"
                                                + entry.getKey() + ";" + entry.getValue();
                                    } else
                                        messagePart1 += "==" + entry.getKey() + ";" + entry.getValue();
                                }

                                for (Map.Entry<String, String> entry : predecessor2Data.entrySet()) {
                                    if (messagePart2.isEmpty()) {
                                        messagePart2 += entry.getKey() + ";" + entry.getValue();
                                    } else
                                        messagePart2 += "==" + entry.getKey() + ";" + entry.getValue();
                                }
                                messageToSend = messagePart1 + "<>" + messagePart2;
                            } else
                    ...
                    else if (messageParts[0].equals(INSERT_SUCCESSOR)) {
                        Log.e("ServerTask", "INSERT_SUCCESSOR");
                        ContentValues values = new ContentValues();
                        String coordinator = messageParts[1];
                        values.put(KEY, messageParts[2]);
                        values.put(VALUE, messageParts[3]);
                ...
                for (String port : recoveryList) {
                    try {
                        ...
                        if (messageReceived.contains(RECOVERY_ACK)) {
                            String[] msg = messageReceived.split(":");
                            if(msg.length > 1) {
                                if (msg[1].equals(dynamoRing.getSuccessor(MY_PORT))) {
                                    ContentValues values = new ContentValues();
                                    ...
                                    for (int j = 0; j < content.length; j++) {
                                        predecessor1Data.put(content[j].split(";")[0],
                                                content[j].split(";")[1]);
                                    ...
                                    dbWriter.insertWithOnConflict(SimpleDynamoContract.MessageEntry.TABLE_NAME, null,
                                            values, SQLiteDatabase.CONFLICT_REPLACE);
                                } else if (msg[1].equals(dynamoRing.getPredecessor(MY_PORT))) {
                                    ContentValues values = new ContentValues();
